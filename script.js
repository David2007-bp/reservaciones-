...
function renderTables(){
  ...
  availableTables.forEach(table => {
    ...
    tableDiv.innerHTML = `<img src="misterwils-blog-novedades-mesas-restaurantes-1.jpg" alt="Mesa ${table.number}"><div class="table-name">Mesa ${table.number}</div>`;
  ...
  reservations.forEach(res => {
    const tableDiv = document.createElement('div');
    tableDiv.className = 'table';
    tableDiv.innerHTML = `<img src="misterwils-blog-novedades-mesas-restaurantes-1.jpg" alt="Mesa ${res.number}"><div class="table-name">Mesa ${res.number}</div>`;
...
function generateReport(){
  ...
  reservations.forEach(res => {
    ...
    reportOutput.textContent += 
      `Mesa ${res.number} - Reservada por: ${res.customer} - Fecha: ${formattedDate} - Hora: ${formattedTime}\n`;
  });
